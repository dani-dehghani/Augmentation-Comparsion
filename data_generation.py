...
def augmented_percent(dataset, aug_method, sampling_percentage):
    ...
    for index in common_rows.index:
        if index + 4 < len(augmented_df):
            indices_to_extract.extend(range(index, index + 5))
    ...
    # Define a file path for the output CSV file
    output_csv_path = os.path.join(augmented_path, f'meth_{aug_method}_{sampling_percentage}_pctwts_0.1_example_4.csv')
...
def augmented_example(dataset, aug_method, sampling_percentage, example):
    data_path = f'data/original/{dataset}'
    original_df = pd.read_csv(os.path.join(data_path, f'train_{sampling_percentage}_percent.csv'))
